...
RSpec.describe Api::ReservesController, type: :controller do
  ...
  describe "create" do
    ...
    context "for a valid reservation with a valid token" do
      before :each do
        WebMock.stub_request(:get, "http://koha.example.com/reserves/create?biblionumber=50&borrowernumber=1&branchcode=10&itemnumber=&password=password&reservenotes=L%C3%A5netyp:%20Heml%C3%A5n&userid=username").
          with(:headers => {'Accept'=>'*/*', 'Accept-Encoding'=>'gzip, deflate', 'Host'=>'koha.example.com'}).
    ...
    context "given a specific response from Koha" do
      ...
      context "when the reserve cannot be placed (http 403)" do
        before :each do
          WebMock.stub_request(:get, "http://koha.example.com/reserves/create?biblionumber=1&borrowernumber=1&branchcode=10&itemnumber=1&password=password&reservenotes=L%C3%A5netyp:%20Heml%C3%A5n&userid=username").
            with(:headers => {'Accept'=>'*/*', 'Accept-Encoding'=>'gzip, deflate', 'Host'=>'koha.example.com'}).
