...
class Print
  ...
  def self.print_segment(pdf, obj, start_cursor)
    ...
    if obj[:barcode]
      ...
      barcode.annotate_pdf(pdf, {:x => 70.send(:mm), :y => start_cursor - 95.send(:mm), :height => 8.send(:mm)})
      pdf.bounding_box([70.send(:mm), start_cursor - 95.send(:mm)], :width => 30.send(:mm),  height: 5.send(:mm), :align=>:right) do
